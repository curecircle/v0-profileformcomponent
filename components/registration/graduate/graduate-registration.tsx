...
import { SpecialtyInfoStep } from "./specialty-info-step"
import { GraduateDocumentsStep } from "./graduate-documents-step"
import { GraduateConfirmationStep } from "./graduate-confirmation-step"
import { ChevronRight, ChevronLeft } from "lucide-react"

const steps = ["اطلاعات پایه", "اطلاعات هویتی", "موقعیت", "اطلاعات تخصصی", "مدارک", "تأیید نهایی"]

export function GraduateRegistration() {
  const [currentStep, setCurrentStep] = useState(1)
  const [formData, setFormData] = useState<Partial<GraduateData>>({})
  const [errors, setErrors] = useState<Record<string, string>>({})

  const updateFormData = (data: Partial<GraduateData>) => {
  ...
  const validateStep = (): boolean => {
    ...
    switch (currentStep) {
      ...
      case 3:
        ...
        break
      case 4:
        if (!formData.specialty) newErrors.specialty = "تخصص الزامی است"
        if (!formData.graduateUniversity) newErrors.graduateUniversity = "دانشگاه الزامی است"
        if (!formData.graduationYear) newErrors.graduationYear = "سال فارغ‌التحصیلی الزامی است"
        break
      case 5:
        if (!formData.diplomaFile) newErrors.diplomaFile = "مدرک تحصیلی الزامی است"
  ...
  const handleSubmit = async () => {
    console.log("[v0] Submitting graduate registration:", formData)
    ...
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="text-center space-y-2">
          <h1 className="text-3xl font-bold text-foreground">ثبت‌نام پزشک فارغ‌التحصیل</h1>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            {currentStep === 4 && <SpecialtyInfoStep data={formData} onChange={updateFormData} errors={errors} />}
            {currentStep === 5 && <GraduateDocumentsStep data={formData} onChange={updateFormData} errors={errors} />}
            {currentStep === 6 && <GraduateConfirmationStep data={formData} />}
