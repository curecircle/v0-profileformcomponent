...
export interface GraduateFormProps {
...
export function GraduateForm({
  ...
  useEffect(() => {
    const fetchOptions = async () => {
      setLoading(true);
      try {
        const [univs, degs, specs] = await Promise.all([
          getUniversities(),
          getDegrees(),
          getSpecialties(),
        ]);
    ...
    <Card className={cn('border-2 shadow-xl animate-in fade-in slide-in-from-right duration-500', className)}>
      <CardHeader className="space-y-2 pb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <CardTitle className="text-xl sm:text-2xl">{t('educationHistory')}</CardTitle>
            <p className="text-sm text-muted-foreground mt-1">
              {t('addCompletedEducation')}
            </p>
          </div>
        </div>
      </CardHeader>
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="text-base sm:text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                  <span className="flex items-center justify-center w-7 h-7 rounded-full bg-blue-100 dark:bg-blue-900 text-blue-600 dark:text-blue-400 text-sm font-bold">
                    {index + 1}
                  </span>
                  {t('education')} {index + 1}
                </h3>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-semibold text-slate-700 dark:text-slate-300">
                    {t('endYear')} <span className="text-red-500">*</span>
                  </label>
                  <select
